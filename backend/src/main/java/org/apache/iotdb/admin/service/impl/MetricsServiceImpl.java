...
import org.apache.iotdb.admin.common.ApplicationContextGetBeanHelper;
import org.apache.iotdb.admin.common.enums.JVMConvertEnum;
import org.apache.iotdb.admin.common.enums.JVMIndicatorsEnum;
...
public class MetricsServiceImpl implements MetricsService {
  ...
  public JVMMetricsListDataVO getYGCHappendCountAndCostTime(Connection connection)
    ...
    String countSQL =
        "select * from "
            //            + "root._metric.\"0.0.0.0:"
            //            + port
            //            + "\".\"jvm.gc.pause_count\".\"action=end of minor GC\".\"cause=Allocation
            // Failure\", "
            + "root._metric.\"0.0.0.0:"
            + port
    ...
    String timeSQL =
        "select * from "
            //            + "root._metric.\"0.0.0.0:"
            //            + port
            //            + "\".\"jvm.gc.pause_total\".\"action=end of minor GC\".\"cause=Allocation
            // Failure\", "
            + "root._metric.\"0.0.0.0:"
            + port
    ...
    //    double time =
    //        (Double.parseDouble(timeValues.get(1)) + Double.parseDouble(timeValues.get(2))) /
    // 1000;
  ...
  public JVMMetricsListDataVO getYGCMaxCostTimeAndReason(Connection connection)
    ...
    Integer detailAvailable = 2;
    String latestResult =
        Double.parseDouble(timeValues.get(1)) / 1000 + "s(Metadata GC Threshold)、";
  ...
  public JVMMetricsListDataVO getFGCHappendCountAndCostTime(Connection connection)
    ...
    if (countValues != null && countValues.size() > 0) {
      ...
      //    double d2 = timeValues.get(2).equals("null")? 0.0 :
      // Double.parseDouble(timeValues.get(2));
      //    double d3 = timeValues.get(3).equals("null")? 0.0 :
      // Double.parseDouble(timeValues.get(3));
  ...
  public JVMMetricsListDataVO getJVMCommittedMemorySize(Connection connection)
    ...
    String sql =
        "select * from "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=nonheap\".\"id=CodeHeap 'non-profiled nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=nonheap\".\"id=CodeHeap 'non-nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=nonheap\".\"id=Metaspace\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=nonheap\".\"id=CodeHeap 'profiled nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=heap\".\"id=G1 Old Gen\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=heap\".\"id=G1 Survivor Space\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.committed.bytes\".\"area=heap\".\"id=G1 Eden Space\" "
            + "order by time desc limit 1";
    SessionPool sessionPool = getSessionPool(connection);
    List<String> values = executeQueryOneLine(sessionPool, sql);
    long lastestTimeStamp = Long.parseLong(values.get(0));
  ...
  public JVMMetricsListDataVO getJVMMemoryMaxSize(Connection connection) throws BaseException {
    ...
    String sql =
        "select * from "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=nonheap\".\"id=CodeHeap 'non-profiled nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=nonheap\".\"id=CodeHeap 'non-nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=nonheap\".\"id=Metaspace\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=nonheap\".\"id=CodeHeap 'profiled nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=heap\".\"id=G1 Old Gen\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=heap\".\"id=G1 Survivor Space\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.max.bytes\".\"area=heap\".\"id=G1 Eden Space\" "
            + "order by time desc limit 1";
    SessionPool sessionPool = getSessionPool(connection);
    List<String> values = executeQueryOneLine(sessionPool, sql);
    long lastestTimeStamp = Long.parseLong(values.get(0));
  ...
  public JVMMetricsListDataVO getJVMMemoryUsedSize(Connection connection) throws BaseException {
    ...
    String sql =
        "select * from "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=nonheap\".\"id=CodeHeap 'non-profiled nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=nonheap\".\"id=CodeHeap 'non-nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=nonheap\".\"id=Metaspace\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=nonheap\".\"id=CodeHeap 'profiled nmethods'\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=heap\".\"id=G1 Old Gen\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=heap\".\"id=G1 Survivor Space\", "
            + "root._metric.\"0.0.0.0:"
            + port
            + "\".\"jvm.memory.used.bytes\".\"area=heap\".\"id=G1 Eden Space\" "
            + "order by time desc limit 1";
    SessionPool sessionPool = getSessionPool(connection);
    List<String> values = executeQueryOneLine(sessionPool, sql);
    long lastestTimeStamp = Long.parseLong(values.get(0));
  ...
  public List<MetricsListDataVO> getJVMMetricsDataList(Connection connection) throws BaseException {
    ...
    //    list = getJVMMetricsDataListNew( connection);

    list = getJVMMetricsDataListNew2(connection);
    return list;
  }

  public List<MetricsListDataVO> getJVMMetricsDataListNew2(Connection connection)
      throws BaseException {
    SessionPool sessionPool = getSessionPool(connection);
    int port = connection.getPort();
    Integer detailAvailable = 0;
    ...
    List<MetricsListDataVO> list = new ArrayList<>();
    JVMConvertEnum[] values = JVMConvertEnum.values();
    long nowDate = new Date().getTime(); // s
    long levelTime = nowDate - 15 * 16 * 1000;
    String pattern = "yyyy-MM-dd' 'HH:mm:ss.SSS";
    SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
    String datePattern = "yyyy-MM-dd'T'HH:mm:ss";
    SimpleDateFormat dateFormat = new SimpleDateFormat(datePattern);
    String levelTimeFormat = dateFormat.format(levelTime);
    String nowTimeFormat = dateFormat.format(nowDate);
    for (JVMConvertEnum value : values) {
      MetricsListDataVO metricsListDataVO = new MetricsListDataVO();
      metricsListDataVO.setName(value.getName());
      if (StringUtils.isNotBlank(value.getDefaultValue())) {
        metricsListDataVO.setLatestResult(value.getDefaultValue());
      }
      String sql = " select sum(*) from  " + "root._metric.\"0.0.0.0:" + port + "\".\"";
      sql +=
          value.getMean()
              + "\".* where time > "
              + levelTimeFormat
              ...
              + nowTimeFormat
              + "),15s),level="
              + value.getQuerySQLLevel();
      List<String> result = executeQueryOneLine(sessionPool, sql);
      long lastestTimeStamp = new Date().getTime();
      if (!CollectionUtils.isEmpty(result)) {
        lastestTimeStamp = Long.parseLong(result.get(0));
      } else {
        sql =
            "select * from "
                + "root._metric.\"0.0.0.0:"
                ...
                + value.getMean()
                + "\" order by time desc limit 1";
        result = executeQueryOneLine(sessionPool, sql);
        if (!CollectionUtils.isEmpty(result)) {
          lastestTimeStamp = Long.parseLong(result.get(0));
        }
      }
      String methodName = value.getImplClass();
      JVMIndicatorsConvert jvmIndicatorsConvert =
          (JVMIndicatorsConvert) ApplicationContextGetBeanHelper.getBean(methodName);
      String resultValue = jvmIndicatorsConvert.getResultValue(result, value.name());
      metricsListDataVO.setLatestResult(resultValue);
      metricsListDataVO.setDetailAvailable(value.getDetailAvailable());
      String latestScratchTime = simpleDateFormat.format(lastestTimeStamp);
      metricsListDataVO.setLatestScratchTime(latestScratchTime);
      metricsListDataVO.setMetricType(value.getMetricType());
      list.add(metricsListDataVO);
    }
    return list;
  }

  public List<MetricsListDataVO> getJVMMetricsDataListNew(Connection connection)
      throws BaseException {
    SessionPool sessionPool = getSessionPool(connection);
    int port = connection.getPort();
    Integer detailAvailable = 0;
    ...
    List<MetricsListDataVO> list = new ArrayList<>();
    //                + +\".* where time > 2022-10-18T17:20:00 group by ([2022-10-18T17:20:09,
    // 2022-10-18T17:23:54),15s),level=1,2,3";
    // select sum(*) from  root._metric."0.0.0.0:6667"."jvm.buffer.memory.used.bytes".* where time >
    // 2022-10-18T17:20:00 group by ([2022-10-18T17:20:09, 2022-10-18T17:23:54),15s),level=1,2,3
    JVMIndicatorsEnum[] values = JVMIndicatorsEnum.values();
    long nowDate = new Date().getTime(); // s
    long levelTime = nowDate - 15 * 16 * 1000;
    String pattern = "yyyy-MM-dd' 'HH:mm:ss.SSS";
    SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
    ...
    for (JVMIndicatorsEnum value : values) {
      ...
      if (StringUtils.isNotBlank(value.getTemplate())) {
        ...
        String[] valueNew = new String[split.length];
        for (int i = 0; i < split.length; i++) {
          int j = Integer.parseInt(split[i]);
          if (result.size() > j) {
            valueNew[i] = result.get(j);
          } else {
            valueNew[i] = "";
          }
        }
        formated = String.format(valueTemplate, valueNew);
        metricsListDataVO.setLatestResult(formated);
      }
      metricsListDataVO.setDetailAvailable(value.getDetailAvailable());
      String latestScratchTime = simpleDateFormat.format(lastestTimeStamp);
      metricsListDataVO.setLatestScratchTime(latestScratchTime);
